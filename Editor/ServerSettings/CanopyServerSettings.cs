...
public class CanopyServerSettings : EditorWindow
{
    public string ProjectID = "Enter Your Project ID";
    public string ChainID = "Enter Your Chain ID";
    public string Chain = "Enter Your Chain i.e Ethereum, Binance, Cronos";
    public string Network = "Enter Your Network i.e Mainnet, Testnet";
    public string RPC = "Enter Your RPC";
    ...
    void Awake()
    {
        if ((ProjectID == ("Enter Your Project ID")) && (PlayerPrefs.GetString("ProjectID") != ""))
        {
            ProjectID = PlayerPrefs.GetString("ProjectID");
            PlayerPrefs.Save();
        }

        if ((ChainID == ("Enter Your Chain ID")) && (PlayerPrefs.GetString("ChainID") != ""))
        {
            ChainID = PlayerPrefs.GetString("ChainID");
            PlayerPrefs.Save();
        }

        if (Chain == ("Enter Your Chain i.e Ethereum, Binance, Cronos") && (PlayerPrefs.GetString("Chain") != ""))
        {
            Chain = PlayerPrefs.GetString("Chain");
            PlayerPrefs.Save();
        }

        if (Network == ("Enter Your Network i.e Mainnet, Testnet") && (PlayerPrefs.GetString("Network") != ""))
        {
            Network = PlayerPrefs.GetString("Network");
            PlayerPrefs.Save();
        }

        if (RPC == ("Enter Your RPC") && (PlayerPrefs.GetString("RPC") != ""))
        {
    ...
    // Initializes window
    [MenuItem("Window/CanopyServerSettings")]
    public static void ShowWindow()
    {
        // show existing window instance. If one doesn't exist, make one.
        EditorWindow.GetWindow(typeof(CanopyServerSettings));
    }

    // called when menu is opened, loads Canopy Logo
    void OnEnable()
    {
        m_Logo = (Texture2D)Resources.Load("canopybanner", typeof(Texture2D));
    }
    ...
    void OnGUI()
    {
        ...
        GUILayout.Label(m_Logo, GUILayout.MaxWidth(800f), GUILayout.MaxHeight(176f));
        EditorGUILayout.EndVertical();
        // text
        GUILayout.Label("Welcome To The Canopy SDK!", EditorStyles.boldLabel);
        ...
        {
            // Registration form
            Application.OpenURL("https://beta.canopy.tools/");
        }
        // save button
        ...
        GUILayout.Label("Reminder: Your ProjectID must be valid to save & build with our SDK. You can register for one at: https://beta.canopy.tools", EditorStyles.label);
    }
}